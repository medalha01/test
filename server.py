...
class Server:
    ...
    def handle_read_request(self, conn, message):
        ...
        response = {'type': READ_RESPONSE, 'item': item, 'value': value, 'version': version}
        conn.sendall(serialize(response).encode())
        print(f"Server {self.server_id} handled READ_REQUEST for item '{item}': value={value}, version={version}")
    ...
    def handle_commit_request(self, conn, message):
        ...
        with self.lock:
            ...
            if abort:
                response = {'type': COMMIT_RESPONSE, 'transaction_id': transaction['id'], 'status': 'ABORT'}
                if conn:
                    conn.sendall(serialize(response).encode())  # Send response if connection exists
                print(f"Server {self.server_id} aborted transaction {transaction['id']}")
            else:
                # Apply the write set
                for write in ws:
                ...
                if conn:
                    conn.sendall(serialize(response).encode())  # Send response if connection exists
                print(f"Server {self.server_id} committed transaction {transaction['id']} with write set: {ws}")
    ...
    def handle_broadcast_message(self, message):
        ...
        try:
            ...
            if msg_type == COMMIT_REQUEST:
                ...
                self.handle_commit_request(None, msg)  # Simulate the request without a connection
                ...
                if status == 'COMMIT':
                    ...
                    with self.lock:
                        for write in ws:
                            ...
                            _, current_version = self.data_store.get(item, (None, 0))
                            self.data_store[item] = (value, current_version + 1)
                        print(f"Server {self.server_id} applied COMMIT for transaction {transaction_id} with write set: {ws}")
                elif status == 'ABORT':
                    print(f"Server {self.server_id} acknowledged ABORT for transaction {transaction_id}")
            else:
    ...
    def get_data_store(self):
            ...
            with self.lock:
                return dict(self.data_store)  # Return a copy of the data store to avoid race conditions
