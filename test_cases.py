...
def run_tests():
    ...
    operations1 = [
        {"type": "READ", "item": "x"},
        {"type": "WRITE", "item": "x", "value": "A"},
    ]
    operations2 = [
        {"type": "READ", "item": "y"},
        {"type": "WRITE", "item": "y", "value": "B"},
    ]
    threading.Thread(
        target=lambda: Transaction(1, operations1, broadcast, replicas).execute(),
        daemon=True,
    ).start()
    threading.Thread(
        target=lambda: Transaction(2, operations2, broadcast, replicas).execute(),
        daemon=True,
    ).start()
